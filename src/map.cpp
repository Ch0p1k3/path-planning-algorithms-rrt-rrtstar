...
bool Map::buildMap(const char* filePath) {
  ...
  bool hasHeight = false, hasWidth = false, hasSTX = false, hasSTY = false,
       hasFINX = false, hasFINY = false, hasGrid = false;
  ...
  for (pugi::xml_node_iterator it = map.begin(); it != map.end(); ++it) {
    ...
    if (nameNode == CS_TAG_HEIGHT) {
      if (hasHeight) {
        std::cerr
            << "Warning! Repeat tag height. The old value will be used!\n";
      } else {
        hasHeight = true;
      ...
      if (hasSTX) {
        std::cerr
            << "Warning! Repeat tag startx. The old value will be used!\n";
      } else {
        hasSTX = true;
      ...
      if (hasSTY) {
        std::cerr
            << "Warning! Repeat tag starty. The old value will be used!\n";
      } else {
        hasSTY = true;
      ...
      if (hasFINX) {
        std::cerr
            << "Warning! Repeat tag finishx. The old value will be used!\n";
      } else {
        hasFINX = true;
      ...
      if (hasFINY) {
        std::cerr
            << "Warning! Repeat tag finishy. The old value will be used!\n";
      } else {
        hasFINY = true;
  ...
  for (pugi::xml_node row : gridNode.children()) {
    ...
    if (grid[i].size() > width) {
      std::cerr << "Error! Width " << grid[i].size() << " in " << i
                << " line.\n";
      return false;
    }
    ++i;
